...
def init_model():
    ...
    checkpoint = torch.load("F:/study/wxg/6detection/runs/exp1/epoch_009.pth", map_location='cpu')
...
class Pred():
    # 参数初始化
    def __init__(self, model, img_root,device):
        self.model = model
    ...
    def result(self):
        ...
        #这里是我自己制作label时出现了失误打的补丁，因为重新训练有些麻烦，如果是自己制作的人统一用yx顺序可以省略这一块
        temp = Result.clone()
        Result[:,:,1,:,:] = temp[:,:,2,:,:]
        Result[:,:,2,:,:] = temp[:,:,1,:,:]
        Result[:,:,52,:,:] = temp[:,:,53,:,:]
        ...
        bbox1 = self.Decode(1, Result1)
        Result2 = Result[2].permute(0, 2, 3, 1).squeeze(0)
        bbox2 = self.Decode(2, Result2)
        ...
        #keep = batched_nms(bbox[:, :4], bbox[:, 4], bbox[:, 5].long(), iou_threshold=iou_con)
        #bboxes = bbox[keep]
        if len(bboxes) == 0:
            print("未识别到任何物体")
                    ...
                    for n in range(y_area[0], y_area[1]):
                        for m in range(x_area[0], x_area[1]):
                            for c in range(20):
                                ...
                                i_, j_, n_, m_ = result[i, j, 51 * p + 1], result[i, j, 51 * p + 2], result[
                                    n, m, 51 * (p + 2) + 1], result[n, m, 51 * (p + 2) + 2]
                                l_ij_x = result[i, j, 51 * p + 3 + m]
                                l_ij_y = result[i, j, 51 * p + 17 + n]
                                l_nm_x = result[n, m, 51 * (p + 2) + 3 + j]
